...
<script type="text/javascript">
	...
	/* 	 for (var i = 0; i < objArray.length; i++) {
	        ...
	        if(objArray[i]==$('input[name="roleId"]').val()){
	...
	function submit(){
		...
		if($("#username").val()==null ||$("#name").val()==null ||$("#name").val()=='' || $("#email").val()==null || $("#tele").val()==null || sel==null ||sex==null || address==null|| birthday==null){
			alert("请输入完整信息！");
			result=false;
		}else{
			result=true;
	...
	//重置
	function reloads(){
		window.location.href="${pageContext.request.contextPath }/login/toupdate.do?uuid="+${info.uuid};
	}
	
	function quit(){
		//alert("1")
		window.location.href='${base}/login/quit.do';
	}
...
<input type="hidden" value="${rolist.size() }">
<input type="hidden" id="depname" value="${info.depId }">
